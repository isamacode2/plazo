...
export default function MobileNavMenu({ isLoggedIn }: { isLoggedIn: boolean }) {
  ...
  return (
    ...
    <div className="relative sm:hidden">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="flex h-10 w-10 items-center justify-center rounded-md border border-gray-300 text-gray-700"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-12 z-50 w-56 overflow-hidden rounded-md border border-gray-200 bg-white py-1 shadow-lg">
            {isLoggedIn ? (
              <>
                <Link href="/messages" className={itemClass} onClick={() => setOpen(false)}>
                  Messages
                </Link>
